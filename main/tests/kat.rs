...
fn kat_dilithium2() {
    ...
    for (idx, seed) in seeds.iter_mut().enumerate() {
        unsafe {
            ...
            li2::dilithium2_keygen_from_seed(&mut sk_actual, &mut pk_actual, seed).unwrap();
...
fn kat_dilithium3() {
    ...
    for (idx, seed) in seeds.iter_mut().enumerate() {
        unsafe {
            ...
            let mut pk_actual = [0; CRYPTO_PUBLICKEYBYTES as usize];
            let mut sig_actual = [0; CRYPTO_BYTES as usize];
            li2::dilithium3_keygen_from_seed(&mut sk_actual, &mut pk_actual, seed).unwrap();
...
fn kat_dilithium5() {
    ...
    for (idx, seed) in seeds.iter_mut().enumerate() {
        unsafe {
            ...
            let mut pk_actual = [0; CRYPTO_PUBLICKEYBYTES as usize];
            let mut sig_actual = [0; CRYPTO_BYTES as usize];
            li2::dilithium5_keygen_from_seed(&mut sk_actual, &mut pk_actual, seed).unwrap();
